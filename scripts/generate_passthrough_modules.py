...
class Config:
    ...
    def __post_init__(self):
        self.main_module_path = (_src_root / "bioimageio" / "spec" / self.target_rdf).resolve()
        self.versioned_module_path = (
            _src_root / "bioimageio" / "spec" / self.target_rdf / self.target_version
        ).resolve()
...
def get_ordered_version_submodules(target_rdf: str):
    matches: Iterable[re.Match] = filter(
        ...
        [
            ...
            for f in os.scandir(_src_root / "bioimageio" / "spec" / target_rdf)
            if f.is_dir()
        ],
    )
