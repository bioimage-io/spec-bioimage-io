...
def open_zarr_multiscale_array(uri: str) -> tuple[zarr.Array[Any], Multiscale]:
    ...
    if isinstance(zarr_array_or_group, zarr.Group):
        raise ValueError(
            f"{uri} does not point to a zarr array, but a group with keys: {list(ms.keys())}. Append '/<key>' to the URI to open a specific array."
        )

    return zarr_array_or_group, ms


def _recurse_url_to_find_group(uri: str) -> tuple[Multiscale, str, str]:
...
def _ms_from_group(
    ...
    valid_multiscales: list[Multiscale] = []
