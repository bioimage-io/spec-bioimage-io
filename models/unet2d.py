...
class UNet2d(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.encoders = nn.ModuleList([self.conv_layer(self.input_channels, 16),
        ...
        self.decoders = nn.ModuleList([self.conv_layer(128, 64),
                                       self.conv_layer(64, 32),
                                       self.conv_layer(32, 16)])
        self.upsamplers = nn.ModuleList([self.upsampler(128, 64),
                                         self.upsampler(64, 32),
                                         self.upsampler(32, 16)])
    ...
    def conv_layer(self, in_channels, out_channels):
        kernel_size = 3
        padding = 1
        return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size, padding=padding),
